...
uint8_t bsp_WriteCpuFlash(uint32_t _ulFlashAddr, uint8_t *_ucpSrc, uint32_t _ulSize)
{
	...
	for (i = 0; i < _ulSize / 32; i++)	/* 未处理非32字节整数倍个数情况 */
	{
		...
		if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, _ulFlashAddr, ((uint32_t)FlashWord)) == HAL_OK)
		{
	...
	ucRet = bsp_CmpCpuFlash(FlashAddr, pSrc, _ulSize);
