...
print("load \"STM32F446xx_512.lua\" ok")
...
function config_cpu(void)
	...
	ERASE_CHIP_TIME = 8200		--全片擦除时间ms（仅用于进度指示)

	OB_ADDRESS     = "1FFFC000 1FFFC001 1FFFC008 1FFFC009 "

	OB_SECURE_OFF  = "EF AA FF 3F"	--SECURE_ENABLE = 0时，编程完毕后写入该值
	OB_SECURE_ON   = "EF 00 FF 3F"	--SECURE_ENABLE = 1时，编程完毕后写入该值

	--判断读保护和写保护的条件(WRP = Write protection)
	OB_WRP_ADDRESS   = {0x1FFFC001, 0x1FFFC008, 0x1FFFC009} 	--内存地址
	OB_WRP_MASK  	 = {0xFF, 0xFF, 0x80}					--读出数据与此数相与
	OB_WRP_VALUE 	 = {0xAA, 0xFF, 0x00}					--相与后与此数比较，相等表示没有保护
end
