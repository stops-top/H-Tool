/***************************************************************************\
*                                                                           *
*       Unit_YBCommDevice_About.cpp - 串行口通讯关于窗口, 版本 1.5.0.5      *
*                             Built: Jul. 10, 2015                          *
...
{
  bool bIsNT = false;
  DWORD dwMajorVer = 0;  //4    //4.10.1998
  DWORD dwMinorVer = 0;  //10   //4.10.1998
  DWORD dwBuildNum = 0;  //1998 //4.10.1998
  UnicodeString sOSName, sCSDVersion, sCSDBuildNumber;
  ...
  if(GetVersionEx(&ovi))
   {
     bIsNT = ovi.dwPlatformId == VER_PLATFORM_WIN32_NT;
     dwMajorVer = ovi.dwMajorVersion;
     dwMinorVer = ovi.dwMinorVersion;
     dwBuildNum = LOWORD(ovi.dwBuildNumber);
   }
  ...
  if(sOSName.IsEmpty())
    sOSName = _T("Windows");
  sOSName.cat_sprintf(_T(" %d.%02d.%04d"), dwMajorVer, dwMinorVer, dwBuildNum);
...
{
  ShellExecute(Application->Handle, _T("open"), TMsgStrings().Str_CRgtHomePage.w_str(), NULL, _T(""), SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
void __fastcall TForm_YBComm64_About::LabelEmailClick(TObject *Sender)
{
  ShellExecute(Application->Handle, _T("open"), TMsgStrings().Str_CRgtAuMailTo.w_str(), NULL, _T(""), SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
  ...
  static const wchar_t *_Msg_Chs[] =
   {
     ...
     L"用户:"                    , // 2 Str_User
     L"单位:"                    , // 3 Str_Orgn
     L"版本: 1.5.0.3"            , // 4 Str_CRgtVersion
  ...
  static const wchar_t *_Msg_Enu[] =
   {
     ...
     L"User:"                    , //2 Str_User
     L"Orgn:"                    , //3 Str_Orgn
     L"Version: 1.5.0.3"         , //4 Str_CRgtVersion
     L"Copyright © Victor Chen"  , //5 Str_CRgtCoName
     L"http://www.cppfans.com"   , //6 Str_CRgtHomePage
     L"victor@cppfans.com"       , //7 Str_CRgtAuEmail
     L"mailto:victor@cppfans.com", //8 Str_CRgtAuMailTo
