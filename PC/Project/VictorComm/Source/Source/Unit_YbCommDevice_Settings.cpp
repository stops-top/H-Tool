/***************************************************************************\
*                                                                           *
*   Unit_YbCommDevice_Settings.cpp - 串行口通讯参数设定窗口, 版本 1.5.0.5   *
*                             Built: Jul. 10, 2015                          *
...
void __fastcall TForm_YbCommDevice_Settings::BnOKClick(TObject *Sender)
{
  ...
  if(CbPort->ItemIndex<0)
   {
     ...
     Application->MessageBox(Msg.ErrSelPort.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
     return;
   }
  ...
  if(sNewBaud.IsEmpty())
   {
	 ...
	 Application->MessageBox(Msg.ErrSelBaud.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
	 return;
   }
  NewBaud = _wtoi(sNewBaud.w_str());

  //数据位数
  if(CbByteSize->ItemIndex<0)
   {
     CbByteSize->SetFocus(); //"必须选择数据位"
     Application->MessageBox(Msg.ErrSelBits.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
     return;
   }
  ...
  switch(CbParity->ItemIndex)
   {
     ...
     default: CbParity->SetFocus(); //"必须选择奇偶校验"
              Application->MessageBox(Msg.ErrSelCheck.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
              return;
   }
  ...
  switch(CbStopBits->ItemIndex)
   {
     ...
     default: CbStopBits->SetFocus(); //"必须选择停止位"
              Application->MessageBox(Msg.ErrSelStops.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
              return;
   }
  ...
  switch(CbFlow->ItemIndex)
   {
     ...
     default: CbFlow->SetFocus(); //"必须选择数据流控制"
              Application->MessageBox(Msg.ErrSelFlow.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
              return;
   }
  ...
  try
  ...
  catch(Exception &e)
   {
     ...
     Application->MessageBox(Msg.ErrInvAAns.w_str(),Caption.w_str(),MB_OK|MB_ICONSTOP);
     return;
   }
  ...
  try
  ...
  catch(Exception &e)
   {
     ...
     Application->MessageBox(Msg.ErrInvInBuf.w_str(),Caption.w_str(),MB_OK|MB_ICONSTOP);
     return;
   }
  ...
  try
  ...
  catch(Exception &e)
   {
     ...
     Application->MessageBox(Msg.ErrInvOutBuf.w_str(),Caption.w_str(),MB_OK|MB_ICONSTOP);
     return;
   }
  ...
  try
  ...
  catch(Exception &e)
   {
     ...
     CbPort->SetFocus();
     MessageBox(Handle, e.Message.w_str(), Caption.w_str(), MB_OK|MB_ICONSTOP);
   }
}
//---------------------------------------------------------------------------
  ...
  static const wchar_t *_Msg_Enu[] =
   {
     ...
     L"COM port Setup"            , // 2 MsgPortSet
     L"COM port"                  , // 3 MsgPort
     L"Baud"                      , // 4 MsgBaud
     L"Parity"                    , // 5 MsgCheck
