...
/*
*********************************************************************************************************
*	函 数 名: AnsiStrToChars
*	功能说明: 将ASCII码字符串转换成二进制。 遇到小数点自动越过。
*	形    参: _pStr :待转换的ASCII码串. 可以以逗号，#或0结束。 2014-06-20 修改为非0-9的字符。
*	返 回 值: 二进制整数值
*********************************************************************************************************
*/
...
*********************************************************************************************************
*	函 数 名: BexToAscii
*	功能说明: 1个Hex整数转换为1个ASCII字符
*	形    参: _hex : 整数. 0-15.
*	返 回 值: ASCII字符 0-9， A-F
*********************************************************************************************************
*/
...
void HexBufToAscii(uint8_t *_pHex, char *_pAscii, uint16_t _BinBytes)
{
    ...
    else
    {
        ...
        {
			_pAscii[2 * i] = BcdToChar(_pHex[i] >> 4);
			_pAscii[2 * i + 1] = BcdToChar(_pHex[i] & 0x0F);
        }
		_pAscii[2 * (i - 1) + 2] = 0;
    }
}
...
/*
*********************************************************************************************************
*    函 数 名: AnsiStringToLuaSting
*    功能说明: 将字符串转换为 lua字符串  ”01 02 03 04" -> "\x01\x02\x03\x04"
*    形    参:
*    返 回 值: 转换得到的整数
*********************************************************************************************************
*/
AnsiString AnsiStringToLuaSting(AnsiString s_in)
{
	...
	for (i = 0; i < s_in.Length(); i++)
	{
		...
		|| (p[i] >= 'a' && p[i] <= 'f')  )
		{
			...
			if (half == 0)
			{
				s_out = s_out + "\\x" + (AnsiString)buf;
				half = 1;
			}
			else
			{
				half = 0;
				s_out = s_out + (AnsiString)buf;
			}
		}
	}
	return s_out;
