...
//static int lua_uart_WriteDOex(lua_State* L);
//static int lua_uart_ReadDOex(lua_State* L);
//static int lua_uart_ReadDIex(lua_State* L);
...
static int lua_uart_send(lua_State* L)
{
    size_t len;
	const char *data;
	int port;   /* com号: 支持 COM1, COM4, COM7, COM8 */
	...
	if (port == 1)
	{
		RS485_SendBuf1((uint8_t *)data, len);
	}
	else
	{
		RS485_SendBuf1((uint8_t *)data, len);
...
static int lua_uart_recive(lua_State* L)
{
    ...
    while (1)
    {
		...
		len = Form1->YbCommDevice1->ReadPackage(g_pBuf, BUFFER_SIZE);
		if (len > 0)
		{
        	break;
		}
    ...
    lua_pushlstring(L, (char *)g_pBuf, len);
    return 2;   /* 返回2个参数 */
}
...
static int uart_ReadModbusAck(uint8_t _port, uint8_t *_rx_buf, uint16_t _buf_size, uint16_t _timeout)
{
    ...
    while (1)
    {
        ...
        if (bsp_CheckRunTime(time1) >= (int32_t)_timeout)
        {
            break;  /* 超时 */
        }
    }
...
static int lua_uart_WriteRegFloat(lua_State* L)
{
    ...
    uint16_t addr485;
    uint16_t regaddr;
    float value[WRITE_REG_MAX_NUM / 2];
    uint8_t regnum;
    uint16_t timeout;
    uint8_t txbuf[WRITE_REG_MAX_NUM * 2 + 5];
    uint8_t rxbuf[32];
    ...
    for (i = 0; i < regnum / 2; i++)
    {
		FloatToBEBuf(value[i], &txbuf[pos]);
		pos += 4;
    }
    crc = CRC16_Modbus(txbuf, pos);
    txbuf[pos++] = crc >> 8;
