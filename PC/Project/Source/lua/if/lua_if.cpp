...
//	Form1->Timer1->Enabled = false;	/* 禁止主程序读串口　*/
//
//	Form1->YbCommDevice1->ReadPackage(g_pBuf, BUFFER_SIZE);

	if (luaL_dostring(g_Lua, buf) != LUA_OK)
	{
		AnsiString s;

		s = AnsiString(lua_tostring(g_Lua, -1));
		Form1->MemoLuaOut->Lines->Add(s);

		lua_pop(g_Lua,1); //将错误信息出栈
	}
//	Form1->Timer1->Enabled = true;	/* 允许主程序读串口　*/
...
{
	//Form1->Memo3->Lines->Add((AnsiString)_str);
}
...
*********************************************************************************************************
*/
extern void lua_swd_RegisterFun(void);
static void lua_RegisterFunc(void)
{
...
//	lua_register(g_Lua, "read_clock", read_clock);
	...
	lua_register(g_Lua, "ShowMessage", l_ShowMessage);

	lua_register(g_Lua, "print", l_my_print);

	// 避免加载lua告警 ，这些swd相关函数都是空语句
	lua_swd_RegisterFun();
}
